...
def groups():
    ...
    for group in groups:
        group['members'] = requests.get(f'{API_BASE_URL}/groups/{group["id"]}', headers=headers).json()
...
def add_habit():
    ...
    try:
        response = requests.post(
            ...
            json={
                'name': payload['name'],
                'score_amount': payload['score_amount'],
...
def complete_task(task_id):
    ...
    headers = {'Authorization': f'Bearer {session["token"]}'}
    response = requests.post(f'{API_BASE_URL}/complete_task', headers=headers, json={"task_id": task_id})
    print(response.text)
    if response.status_code == 200:
        return redirect(url_for('dashboard'))
    else:
        return render_template('error.html', error='Ошибка при завершении задачи')
